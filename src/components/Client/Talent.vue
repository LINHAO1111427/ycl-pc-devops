    ...
    <div class="easy">
        <div class="talents-container">
            ...
            <div class="client-form">
                <h1 class="client-form-h1">发现人才</h1>
                <p class="client-form-p">量身定制的人才匹配，帮助您更快地招聘到合适的人选</p>

                <h1 class="client-form-h1">最佳人才匹配</h1>
                <el-scrollbar>
                ...
                <h1 class="client-form-h1">按类别浏览</h1>
                <div class="talent-type" v-for="item,index in data.title" :key="index">
                    <span @click="change_title_id(index)"
                        :class="index == data.title_id ? 'talent-type-title-success':'talent-type-title'">{{ item
                        }}</span>
                    <div class="talent-type-tag" v-if="index == data.title_id">
                        <n-tag size="large" v-for="item,index in data.tag" class="easy-tag" :color="{textColor:'#808080'}"
                            round>
                            {{ item.name }}
                        </n-tag>
                    </div>
...
const data = reactive({
    menu: 1,
    title_id: 0,
    title: ['会计与咨询', '管理员支持', '人工智能服务', '顾客服务', '数据科学与分析', '设计与创意', '工程与建筑', 'IT与网络', '法律', '销售与营销', '译本', 'Web、移动软件开发', '写作', '查看所有专业'],
    tag: [
        { name: '会计学' },
        { name: '业务分析与战略' },
        { name: '职业指导' },
